	...
	**/
	...
	public:
		...
		// Be very careful when using this because it gives access to the actual item when using const Pool, basically bypassing constness
	...
	public:
		...
		Pool<Unit*, 3 * PLAYERS_COUNT + TANKERS_BY_PLAYER_MAX * PLAYERS_COUNT> units; // Reference already created units
		...
		void linkUnits()
		{
			...
			for (int i = 0; i < PLAYERS_COUNT; ++i)
			{
				this->units.add(&this->players[i].reaper);
				this->units.add(&this->players[i].destroyer);
				this->units.add(&this->players[i].doof);
			}

			for (int i = 0; i < this->tankers.currentSize; ++i)
			{
				this->units.add(&this->tankers[i]);
			}
		}
		...
		Collision getNextCollision() 
		{
			...
			for (int i = 0; i < this->units.currentSize; ++i)
			{
				// Test collision with map border first
				Collision collision = this->units[i]->getCollision();
				...
				for (int j = i + 1; j < this->units.currentSize; ++j)
				{
					collision = this->units[i]->getCollision(this->units[j]);
		...
		void updateGame()
		{
			...
			for (int i = 0; i < this->tankers.currentSize; ++i)
			{
				...
				else if (distance >= TANKER_SPAWN_RADIUS + this->tankers[i].radius && full)
				{
					...
					// Remove from units
					for (int j = 0; j < this->units.currentSize; ++j)
					{
						if (this->units[j] == &this->tankers[i])
						{
							this->units.remove(j);
							break;
						}
			...
			for (int i = 0; i < this->units.currentSize; ++i)
			{
				while (this->units[i]->mass >= REAPER_SKILL_MASS_BONUS)
				{
					this->units[i]->mass -= REAPER_SKILL_MASS_BONUS;
				}
			}
		...
		void adjust()
		{
			for (int i = 0; i < this->units.currentSize; ++i)
			{
				this->units[i]->adjust(this->skillEffects);
			}
		}
		...
		void playCollision(const Collision& collision) 
		{
			...
			else
			{
				...
				if (dead) 
				{
					...
					// Remove from tankers
					for (int j = 0; j < this->tankers.currentSize; ++j)
					{
						if (&this->tankers[j] == dead)
						{
							this->tankers.remove(j);
							break;
						}
					}
				}
	...
	{
		this->duration -= 1;
		for (int i = 0; i < board.units.currentSize; ++i)
		{
			if (this->isInRange(*board.units[i], this->radius + board.units[i]->radius))
			{
				this->apply(board.units[i]);
			}
		}
	}
